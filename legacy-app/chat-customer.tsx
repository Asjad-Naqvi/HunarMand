...
export default function ChatCustomerScreen() {
  ...
  const [messages, setMessages] = useState([
    { role: 'agent', text: 'Hello! I am your AI assistant. What kind of service do you need today? (e.g. AC repair, plumbing)' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.18:5000/api/agent/process', {
        ...
        body: JSON.stringify({ message: userMsg, mode: 'customer' })
      });
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Customer Support AI</Text>
        <View style={{ width: 60 }} />
      </View>
